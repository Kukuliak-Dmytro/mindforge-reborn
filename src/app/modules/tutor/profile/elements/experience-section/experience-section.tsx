...
import {
  createTutorExperienceSchema,
  type ITutorExperienceFormData,
...
export const ExperienceSection = ({ experiences }: IExperienceSectionProps) => {
  ...
  const [initialFormValues, setInitialFormValues] =
    useState<ITutorExperienceFormData | null>(null);
  const updateProfile = useUpdateTutorProfile();

  const defaultFormValues: ITutorExperienceFormData = useMemo(
    () => ({
      institution: "",
      title: "",
      startDate: "",
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<ITutorExperienceFormData>({
    resolver: zodResolver(createTutorExperienceSchema(t)),
    defaultValues: defaultFormValues,
    mode: "onChange",
  });

  const handleAdd = (data: ITutorExperienceFormData) => {
  ...
  const handleEdit = (entry: ITutorExperience) => {
    const formValues: ITutorExperienceFormData = {
  ...
  const handleUpdate = (id: string, data: ITutorExperienceFormData) => {
